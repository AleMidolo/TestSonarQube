    ...
    for movie in self.movies:
        if movie['name'].lower() == name.lower():
            for seat in seats_to_book:
                row, col = seat
                if movie['seats'][row][col] == 0:  # Check if the seat is available
                    movie['seats'][row][col] = 1  # Mark the seat as booked
                else:
                    return 'बुकिंग विफल।'  # Seat already booked
            return 'बुकिंग सफल।'  # All seats booked successfully
    return 'फिल्म नहीं मिली।'  # Movie not found