class Warehouse:
    ...
    def create_order(self, order_id, product_id, quantity):
        if self._is_order_valid(product_id, quantity):
            self._process_order(order_id, product_id, quantity)
        else:
            return False

    def _is_order_valid(self, product_id, quantity):
        return self.get_product_quantity(product_id) >= quantity

    def _process_order(self, order_id, product_id, quantity):
