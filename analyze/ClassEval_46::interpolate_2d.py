...
def interpolate_2d(x, y, z, x_interp, y_interp):
    ...
    x_indices = {val: idx for idx, val in enumerate(x_sorted)}
    y_indices = {val: idx for idx, val in enumerate(y_sorted)}
    z_grid = np.zeros((len(y_sorted), len(x_sorted)))
    for i in range(len(x)):
        xi = x[i]
        yi = y[i]
        zi = z[i] if isinstance(z[i], (int, float)) else z[0][i]
        z_grid[y_indices[yi], x_indices[xi]] = zi
    result = []
    for xi, yi in zip(x_interp, y_interp):
        ...
        z_interp = z_x1 + (z_x2 - z_x1) * (yi - y1) / (y2 - y1)
